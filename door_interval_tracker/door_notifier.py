...
load_dotenv()
logger = logging.getLogger("DoorNotifier")


class DoorNotifier:
    """
    Formats and dispatches Panel Door Open Interval Delta Alerts to Google Chat Spaces.
    Highlights:
    - 🚨 NEW Panel Door Opens detected in this specific interval (with exact location & voltages)
    - 🟢 Closed / Recovered Panels (Count summary only)
    - 🟡 Ongoing Door Open Panels
    ...
    def __init__(self, webhook_url: Optional[str] = None, region: str = "central"):
        ...
        if self.region == "north":
            ...
            raw_url = (
                ...
                or os.getenv("CENTRAL_DOOR_GOOGLE_CHAT_WEBHOOK_URL")
                or os.getenv("CENTRAL_ZONE_TELEMETRY_CHAT_WEBHOOK_URL")
                or os.getenv("CENTRAL_ZONE_GOOGLE_CHAT_WEBHOOK_URL")
                or os.getenv("DOOR_GOOGLE_CHAT_WEBHOOK_URL")
                or os.getenv("PANEL_DOOR_GOOGLE_CHAT_WEBHOOK_URL")
                or os.getenv("ZONES_GOOGLE_CHAT_WEBHOOK_URL")
                or os.getenv("GOOGLE_CHAT_WEBHOOK_URL")
            )
        self.webhook_url = str(raw_url).strip() if raw_url else None

    def build_door_delta_report(self, delta_results: Dict[str, Any]) -> str:
        """Constructs formatted Google Chat message with delta breakdown for panel door open."""
        ...
        total_new = delta_results.get("total_newly_opened", 0)
        total_rec = delta_results.get("total_recovered", 0)
        total_ongoing = delta_results.get("total_ongoing", 0)
        total_curr = delta_results.get("total_current_open", 0)

        lines = [
            f"🚪 *BBMP {region_title} — Panel Door Open Interval Tracker*",
        ...
        lines.append("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
        if total_new > 0:
            lines.append(f"🚨 *NEW Door Opens:* *{total_new}*  |  🟢 *Closed/Recovered:* *{total_rec}*  |  🟡 *Ongoing:* *{total_ongoing}*")
        elif total_rec > 0:
            lines.append(f"✅ *No New Door Opens*  |  🟢 *Closed/Recovered:* *{total_rec}*  |  🟡 *Ongoing:* *{total_ongoing}*")
        elif total_curr == 0:
            lines.append(f"✨ *ALL CLEAR:* All panel doors closed across all {region_title} zones!")
        else:
            lines.append(f"ℹ️ *Total Active Door Opens:* *{total_curr}* (No status changes in this interval)")

        lines.append("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n")

        # 1. SECTION: NEWLY OPENED PANELS (CRITICAL ALERT)
        all_new_panels = []
        ...
        if all_new_panels:
            lines.append("🚨 *NEWLY DETECTED DOOR OPENS IN THIS INTERVAL:*")
            ...
            lines.append("🟢 *New Door Opens:* 0 new door open events detected in this interval.")
            lines.append("")
    ...
    def send_door_report(
        ...
        message_text = self.build_door_delta_report(delta_results)
        ...
        for attempt in range(max_retries):
            try:
                logger.info("Sending Door open interval alert to Google Chat...")
        ...
        logger.error(f"Failed to dispatch Door report to Google Chat after {max_retries} attempts.")
        return False
